...
const buildPrompt = ({
  ...
  '?쒓뎅 紐⑤컮???쇳븨紐곗슜 ?곸꽭?섏씠吏 JSON留??앹꽦?섏꽭??',
  '?섏씠吏 ??? 860px?닿퀬 ?ㅻ쭏?몄뒪?좎뼱, 荑좏뙜, 移댄럹24??遺숈뿬?ｋ뒗 寃껋쓣 ?꾩젣濡??⑸땲??',
  `紐⑺몴 ?섏씠吏 ?섎뒗 ${pageCount}?μ엯?덈떎.`,
  `?섏씠吏 ?섎퀎 湲곕? ?덉쭏? ${getPageQualityNote(pageCount)} ?낅땲??`,
  `?섏씠吏 ?섎퀎 ?뱀뀡 洹쒖튃? ?ㅼ쓬怨?媛숈뒿?덈떎: ${JSON.stringify(buildSectionOrder(pageCount))}`,
  '諛섎뱶??page_count, section_order, image_role_mapping, sections, generated_copy瑜??ы븿?섏꽭??',
  'sections.type? hero, feature, usage, detail, benefit, ingredient, proof, comparison, cta 以??섎굹留??ъ슜?섏꽭??',
  'sections.image_role? hero, detail, usage留??ъ슜?섏꽭??',
  'Hero ?뱀뀡?먮뒗 ??쒖뻔???ㅺ퀬, detail ?뱀뀡? ?뷀뀒?쇱뻔, usage ?뱀뀡? ?ъ슜而룹씠 ?ㅻ룄濡?image_role_mapping??諛곗튂?섏꽭??',
  '?대?吏媛 遺議깊븯硫??ъ궗?⑹쓣 ?꾩젣濡?媛???⑸━?곸씤 留ㅽ븨??留뚮뱶?몄슂.',
  `?곹뭹紐? ${productName}`,
  `媛寃??듭뀡: ${price || '誘몄엯??}`,
  `援ъ“??媛寃??뺣낫: ${pricing ? JSON.stringify(pricing) : '?놁쓬'}`,
  `?源?怨좉컼: ${audience || '誘몄엯??}`,
  `???留??ъ씤?? ${sellingPoints || '誘몄엯??}`,
  `?뚮쭏: ${theme}`,
  `異붽? ?꾨＼?꾪듃: ${prompt || '?곹뭹 ?ъ쭊??諛뷀깢?쇰줈 援щℓ ?꾪솚???곸꽭?섏씠吏瑜??앹꽦'}`,
  `?낅줈???대?吏 ?? ${imageCount}`,
  'generated_copy.feature_descriptions???꾩씠肄섑삎 feature ?뱀뀡??諛붾줈 ?????덇쾶 吏㏐퀬 媛뺥븳 臾몄옣?쇰줈 ?묒꽦?섏꽭??',
  'generated_copy.seo_title? ?ㅻ쭏?몄뒪?좎뼱, 荑좏뙜 ?곹뭹紐낆쿂??移댄뀒怨좊━ ?ㅼ썙?쒖? ?듭떖 ?띿꽦???먯뿰?ㅻ읇寃??욎씠寃??묒꽦?섏꽭??',
  'generated_copy.cta??generic ?섏? ?딄쾶 諛곗넚, ?쒖젙?? 泥닿컧 ?댁젏 ???꾪솚 ?붿냼瑜??댁븘 ?묒꽦?섏꽭??',
  '5??援ъ꽦? 援곕뜑?붽린 ?놁씠 ?듭떖留? 7?μ? 媛???쒖??곸씤 洹좏삎?? 10?μ? premium and complete ?먮굦?쇰줈 李⑤퀎?뷀븯?몄슂.',
  '?뱀뀡 媛?臾멸뎄瑜?諛섎났?섏? 留먭퀬, ?뱁엳 10??援ъ꽦?먯꽌??benefit, ingredient, comparison, proof媛 ?쒕줈 ?ㅻⅨ ??븷??媛吏寃??섏꽭??',
  '留덊겕?ㅼ슫, HTML, ?ㅻ챸臾??놁씠 JSON留?異쒕젰?섏꽭??'
].join('\n');
